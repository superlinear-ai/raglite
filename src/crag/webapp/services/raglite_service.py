...
def run_raglite(
    ...
    *,
    ...
    def _execute() -> tuple[list[Any], str, int]:
        messages = [
            {
                "role": "system",
        ...
        if model.use_agentic_rag:
            ...
            if model.use_rerank:
                ...
                chunk_spans = retrieve_context(
                    query=query, num_chunks=num_chunks, config=model.config
                )

            messages.append(
                add_context(user_prompt=query, context=chunk_spans, config=model.config)
            )
        ...
        return chunk_spans, answer, tool_calls

    if capture_logs:
        with redirect_stdout(stdout_capture), redirect_stderr(stderr_capture):
            chunk_spans, answer, tool_calls = _execute()
    else:
        chunk_spans, answer, tool_calls = _execute()
