...
def test_raglite_cli_import_without_llama_cpp(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test that importing the CLI does not require llama-cpp-python."""
    # Unimport raglite and llama_cpp.
    module_names = list(sys.modules)
    for module_name in module_names:
        if module_name.startswith(("llama_cpp", "raglite", "sqlmodel")):
    ...
    # Verify that importing the CLI does not raise an error.
    importlib.import_module("raglite._cli")
